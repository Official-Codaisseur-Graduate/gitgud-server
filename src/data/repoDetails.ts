  ...
  `
    ...
    .then(res => {
      ...
      const dependenciesExistInDescription = () => {
        const pkg_Json = res.data.repository.object.entries.find(
          en => en.name === "package.json"
        );
        if (pkg_Json) {
          ...
          const dependencies = firstSlice
            .slice(index_Start, index_Last)
            .split(",");
          return dependencies
            .map(d => d.split(":")[0])
            .map(x => x.split('"')[1])
      ...
      const sendDescriptionDetails = () => {
        let descriptionDetails = {
          tooLong: false,
          tooShort: false,
          includesDependencies: dependenciesExistInDescription(),
          exists: repoDescription ? true : false
        };
      ...
      const calculateDescriptionScore = () => {
        ...
        if (!dependenciesExistInDescription()) score -= 40;
